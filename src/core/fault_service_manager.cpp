...
namespace ros2_fault_injection
{
  ...
  FaultServiceManager::FaultServiceManager(
      ...
      FaultEventPublisher &events, core::FaultEventRecorder &fault_event_recorder, ReloadScenarioCallback reload_scenario_callback,
      ...
      : node_(node), injectors_(injectors), events_(events),
        ...
        fault_event_recorder_(fault_event_recorder)
  {
      ...
      std::shared_ptr<srv::GetFaultSchema::Response> response)
  {
    ...
    for (const auto &field : schema)
    {
      ...
      response->default_values.push_back(field.default_value.has_value() ? field.default_value.value() : "");
