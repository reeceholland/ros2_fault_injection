...
std::shared_ptr<ros2_fault_injection::FaultInjector> create_injector(
    rclcpp::Node &node,
    const ros2_fault_injection::InjectorConfig &config)
{
  if (config.type == "odom")
  {
    return std::make_shared<ros2_fault_injection::OdomFaultInjector>(node, config);
  }

  if (config.type == "scan")
  {
    return std::make_shared<ros2_fault_injection::ScanFaultInjector>(node, config);
  }

  if (config.type == "joint_state")
  {
    return std::make_shared<ros2_fault_injection::JointStateFaultInjector>(node, config);
  }
...
int main(int argc, char **argv)
{
  ...
  {
    auto injector = create_injector(*node, injector_config);
    if (!injector)
    {
      RCLCPP_ERROR(
          node->get_logger(),
          "Unknown injector type '%s' for injector '%s'",
          injector_config.type.c_str(),
          injector_config.id.c_str());
      rclcpp::shutdown();
      return 1;
    }
  ...
  for (const auto &fault : scenario.faults)
  {
    ...
    if (injector_it == injectors.end())
    {
      RCLCPP_WARN(
          node->get_logger(),
          "Fault '%s' targets unknown injector_id '%s', skipping",
          fault.id.c_str(),
  ...
  auto find_injector_for_fault =
      [&injectors](const std::string &fault_id)
      -> std::shared_ptr<ros2_fault_injection::FaultInjector>
      {
  ...
  auto set_fault_state_service = node->create_service<ros2_fault_injection::srv::SetFaultState>(
          ...
          std::shared_ptr<ros2_fault_injection::srv::SetFaultState::Response> response)
      {
        ...
        if (!injector)
        {
          ...
          RCLCPP_WARN(
              node->get_logger(),
              "Received request with unknown fault_id: %s",
  ...
  auto list_faults_service = node->create_service<ros2_fault_injection::srv::ListFaults>(
          ...
          std::shared_ptr<ros2_fault_injection::srv::ListFaults::Response> response)
      {
        ...
        for (const auto &[injector_id, injector] : injectors)
        {
          ...
          response->fault_ids.insert(
              response->fault_ids.end(),
              fault_ids.begin(),
              fault_ids.end());

          const auto active_fault_ids = injector->active_fault_ids();
          response->active_fault_ids.insert(
              response->active_fault_ids.end(),
              active_fault_ids.begin(),
